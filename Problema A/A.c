...
int checkMatch(Piece myPiece, int x, int y, Piece* available_pieces, char myPieceSide, int* score);
...
int main(int argc, char const *argv[]) {
	...
	putOnPosition ((MAX_PIECES*2)/2, (MAX_PIECES*2)/2, available_pieces, 0);
...
int recursive_method (int num_parts, int x, int y, Piece available_pieces[], Piece playing_field[MAX_PIECES*2][MAX_PIECES*2], int score) {
	...
	if (num_parts == 0){
		...
		if (playing_field[x-1][y].seq[0] == -1){ /*Side a*/
			...
			if (checkMatch (playing_field[x][y], x-1, y, available_pieces, 'a', &score) == 1){
				printf("ENTRO NO A\n");
		...
		if (playing_field[x+1][y].seq[0] == -1) { /*Side b
			if (checkMatch (playing_field[x][y], x+1, y, available_pieces, 'b', &score) == 1){
				printf("ENTRO NO B\n");
				recursive_method (num_parts -1, x+1, y, available_pieces, playing_field, score);
			}
		}
		...
		if (x % 2 == y % 2 && playing_field[x][y+1].seq[0] == -1) { /*Side c
			if (checkMatch (playing_field[x][y], x, y+1, available_pieces, 'c', &score) == 1){
				printf("ENTRO NO C1\n");
...
int checkMatch(Piece myPiece, int x, int y, Piece* available_pieces, char myPieceSide, int* score){
	...
	if (found == 1) {
		...
		putOnPosition (x, y, available_pieces, z);
		return 1;
	} else {
		return 0;
	}
